...
Which doc for what: `claude-lens-architecture.md` (how) · `claude-lens-pages.md` (what — its per-page section tables are **binding over the HTML mockups**) · `gates.md` (Report Card gates) · `claude-lens-plan.md` (when — phases, tasks, decisions log) · `claude-lens-phase4-parallelization.md` (Phase 4 start gates, live orchestration, worktrees, recovery, and opt-in maximum-throughput mode).
