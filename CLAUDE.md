...
- `/start-task`, `/plan-requirements`, `/plan-architecture`, `/generate-tasks`, `/implement`, `/review`, `/commit` — the `dev-pipeline` plugin (`dev-pipeline:*`), not project-local. **Plugin 5.0.0 dropped `disable-model-invocation` from every one of its skills**, so Claude may invoke them directly. `/commit` also exists user-level at `~/.claude/skills/commit/` (no invocation flag either) and is what a bare `/commit` resolves to.
- **Invocable is not the same as automatic.** Each skill's own description still says "use only when the user asks" — invoke one when the user asks for that work, in whatever words ("commit this" → `/commit`, "review PR 123" → `/review`), and don't fire one off a bare coding request. Prefer the skill over hand-rolling its job: the pipeline's conventions (commit trailers, branch naming, worktree ports) live inside the scripts.
- `/release-notes` — project-local, `.claude/skills/release-notes/`, and the **one** skill here carrying `disable-model-invocation: true`. Suggest it by name; only the user can trigger it. (The plugin also ships a `dev-pipeline:release-notes`, which is invocable.)
