...
- **Draft locally, publish in one batch.** `/create-issue` writes drafts to `specs/issues/*.md` (frontmatter: `status: draft → ready → filed`), the user edits them until happy, then `scripts/publish.sh` files all `ready` drafts to GitHub in one sequential `gh` run and stamps each with its issue number/URL. Never file issues one-by-one during drafting.
