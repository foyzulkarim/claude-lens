...
| 11  | Investigation is *ordered* page-by-page against `pages.md`'s per-page row tables (walk Dashboard, then Sessions, then Session Detail, ...), not tier-by-tier in isolation from an abstract source scan — but every field a page needs is still verified against real data (Decision 1 stands), and a field defined while walking one page is reused, never redefined, when a later page needs it | (a) Tier-first investigation with no page-ordering discipline (original plan); (b) one independent task per page with a final merge/de-dup pass | (a) leaves a real gap — nothing forces every page's actual field needs to get checked against the model, only that the model is internally correct; (b) risks 11 slightly-divergent definitions of the same field (e.g. "cache hit %") reconciled only at the end, and a per-page task can't have a real done-signal since its correctness depends on the other 10 tasks. Page-ordered investigation inside a single continuous document gets the coverage benefit of (b) without its duplication risk, and preserves (a)'s single ground-truth definition |
