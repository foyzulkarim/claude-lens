...
| `DataTable<T>` | `{ data: T[]; columns: ColumnDef<T, any>[]; isLoading?: boolean; empty?: ReactNode; onRowClick?: (row: T) => void; initialSorting?: SortingState; getRowId?: (row: T) => string; label?: string } & ({ virtualized: true; height: number } \| { virtualized?: false; height?: never })` | Headless TanStack table rendered as mockup-style `<table>`; client-side sorting toggled from header buttons; column `meta: { align?: "right"; mono?: boolean }` drives `.r`/`.num` styling; `label` sets the `<table>`'s `aria-label` for screen-reader table navigation | `isLoading` → skeleton rows (fixed count) with `role="status"`; `data.length === 0` → renders `empty` (defaults to `<EmptyState message="No data" />`); `columns` is `ColumnDef<T, any>[]` (not `unknown`) to accept normal heterogeneous `createColumnHelper` output, mirroring TanStack's own `TableOptions.columns`; `virtualized: true` requires `height` at the type level (discriminated union, not two independent optionals) — the scroll container + `useVirtualizer` use a fixed row-height estimate. Row accessibility: each `<tr>` keeps native row semantics (no `role="button"`); the canonical keyboard-operable action is a real `<button>` in the first `<td>` when `onRowClick` is set, with the `<tr>`'s own `onClick` left as a mouse-only convenience delegate. Under `virtualized`, the table carries `aria-rowcount` (header + all logical rows) and each rendered row carries `aria-rowindex` so assistive tech knows a row's true position even though only the visible range is mounted; the leading/trailing virtualizer padding rows carry `aria-hidden` on their empty `<td>` so they never announce as blank rows. |
