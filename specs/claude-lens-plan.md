...
- [x] **#P0-5 — LICENSE + repo hygiene** *(dissolved 2026-07-10 — absorbed into #P1-1, see decisions log)*
  Choose and commit a license (MIT unless decided otherwise); add `.nvmrc` so contributors and CI agree on the Node version. Both items are Node/package-identity decisions with the same shape as the `engines`/`packageManager` pins already moved P0-5→P1-1 — there's no reason to split them across two tasks when P1-1 is the first task with a root `package.json` to carry a `license` field, and CI (#P1-3) needs `.nvmrc` to exist by the time it runs, which is inside Phase 1 anyway.
  *Acceptance:* superseded — see #P1-1's acceptance for LICENSE + `.nvmrc`.
...
**Exit criteria (revised 2026-07-10 — per-consumer gates, not one Phase-0 gate; see decisions log):** repo root empty of V1 (met); data-model inventory merged (met, #P0-7). Issue tracking scaffolding is done (#P0-6). LICENSE + `.nvmrc` move into #P1-1's exit bar. Fixtures (#P0-3) gate Phase 2 entry, not Phase 1. Package name (#P0-4) gates Phase 5 entry (before #P5-2), not Phase 1. **Phase 1 is unblocked now.**
...
- [ ] **#P1-1 — Scaffold three-root TS package** *(absorbs #P0-5's LICENSE + `.nvmrc`, 2026-07-10 — see decisions log)*
  `shared/`, `server/`, `client/` per §3; strict TypeScript everywhere; production deps limited to the §2 server list; client deps as devDependencies. Root `package.json` carries the `engines` (Node ≥ 18) and `packageManager` pins (moved here from #P0-5) plus a `license` field; add a `.nvmrc` matching the pinned Node version (also from #P0-5 — no root `package.json`/`.nvmrc` exists before this task); add a LICENSE file at repo root (MIT unless decided otherwise). Package `name` is a **placeholder** — `claude-lens` is taken on npm (#P0-4 finding); the real name is decided before #P5-2 and is a two-field rename, not a rescaffold.
  *Acceptance:* `tsc --noEmit` passes across all three roots; dependency lists match §2 (deviations require editing the architecture doc first); `npm pkg get engines packageManager license` returns the pinned values; LICENSE at repo root; `.nvmrc` present.
